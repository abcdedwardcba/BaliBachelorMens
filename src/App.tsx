...
// --- 🔥 FIREBASE CONFIG (已填入你的 Bali Key) ---
  ...
  stays: [
    {
      ...
      image:
        "https://a0.muscache.com/im/pictures/miso/Hosting-53280875/original/c412e622-4414-41d3-9528-7359556b63d6.jpeg?im_w=1200",
    },
  ],
  schedule: [
    {
  ...
  bucketList: [
    {
      ...
      image:
        "https://images.unsplash.com/photo-1595590424283-b8f17842773f?auto=format&fit=crop&q=80&w=400",
      tags: ["Action"],
    },
    {
      id: "b2",
      title: "Surf Lesson",
      image:
        "https://images.unsplash.com/photo-1502680390469-be75c86b636f?auto=format&fit=crop&q=80&w=400",
      tags: ["Beach", "Sport"],
    },
    {
      id: "b3",
    ...
    },
    {
      ...
      title: "ATV Adventure",
      image:
        "https://images.unsplash.com/photo-1551816230-ef5deaed4a26?auto=format&fit=crop&q=80&w=400",
      tags: ["Action"],
    },
    {
      id: "b5",
      title: "Go-Karting",
      image:
        "https://images.unsplash.com/photo-1574526033100-8800539169a3?auto=format&fit=crop&q=80&w=400",
      tags: ["Speed"],
    },
    {
      id: "b6",
      title: "Sunset Yoga",
      image:
        "https://images.unsplash.com/photo-1599447421405-0c3072a73efd?auto=format&fit=crop&q=80&w=400",
      tags: ["Wellness"],
    },
  ],
};
...
const compressAndUpload = (file, maxWidth = 300) => {
...
const NetflixCard = ({ children, className = "" }) => (
  <div
    className={`bg-[#181818] rounded-md border border-[#404040] overflow-hidden ${className}`}
  >
    {children}
  </div>
);
...
const UserSelectScreen = ({ users, onSelect, onUpdateUser }) => {
  const handlePhotoUpload = async (file, userId) => {
    ...
    const updatedUsers = users.map((u) =>
      u.id === userId ? { ...u, avatar: base64 } : u
    );
    onUpdateUser({ users: updatedUsers }); // Fix: pass object matching updateStore
  };
      ...
      <div className="grid grid-cols-2 gap-8 w-full max-w-sm">
        {users.map((u) => (
          ...
          >
            ...
            <span className="text-gray-400 group-hover:text-white text-lg font-normal">
              {u.name}
            </span>
  ...
  const addActivity = (dayId) => {
    const newSchedule = data.schedule.map((d) => {
      ...
      return {
        ...
        activities: [
          ...d.activities,
          { time: "00:00", title: "New Episode", icon: "sun" },
        ],
      };
    });
    updateStore({ schedule: newSchedule });
          ...
          <div key={day.id} className="group">
            <button
              ...
              className={`w-full flex items-center gap-4 p-3 rounded-md transition-all ${
                isExpanded ? "bg-[#2f2f2f]" : "bg-[#181818] hover:bg-[#2f2f2f]"
              }`}
            >
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-gray-400 text-xs uppercase tracking-wide">
                  {day.activities.length} Episodes
                </p>
              </div>
              <ChevronDown
            ...
            {isExpanded && (
              <div className="ml-14 mt-2 space-y-2 border-l-2 border-[#333] pl-4 animate-in slide-in-from-top-2">
                {day.activities.map((act, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3 py-2 group/act"
                  >
                    <input
                      value={act.time}
                      onChange={(e) =>
                        updateActivity(day.id, idx, "time", e.target.value)
                      }
                    ...
                    <input
                      value={act.title}
                      onChange={(e) =>
                        updateActivity(day.id, idx, "title", e.target.value)
                      }
...
const BucketListTab = ({ data, updateStore }) => {
    ...
    const newSchedule = data.schedule.map((d) => {
      if (d.id === selectedDayId) {
        return {
          ...
          activities: [
            ...d.activities,
            { time: selectedTime, title: selectedItem.title, icon: "fire" },
          ],
        };
      }
      return d;
  ...
  const handlePhotoUpload = async (file, itemId) => {
    ...
    const newBucket = data.bucketList.map((b) =>
      b.id === itemId ? { ...b, image: base64 } : b
    );
  ...
  const addNewBucketItem = () => {
    const newItem = {
      ...
      title: "New Activity",
      image:
        "https://images.unsplash.com/photo-1540206395-688085723adb?auto=format&fit=crop&q=80&w=400",
      tags: ["New"],
    };
  ...
  return (
    <div className="pb-24 px-4 pt-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={addNewBucketItem}
          className="bg-[#333] text-white p-2 rounded-full hover:bg-[#444]"
        >
          <Plus size={18} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {data.bucketList?.map((item) => (
          <div
            key={item.id}
            className="relative group bg-[#181818] rounded-md overflow-hidden border border-[#333] transition-all hover:scale-105 hover:border-gray-500 hover:z-10 shadow-lg"
          >
            <div className="aspect-[2/3] relative">
              ...
              <div className="absolute bottom-14 left-2 flex gap-1">
                {item.tags?.map((t) => (
                  <span
                    key={t}
                    className="text-[9px] bg-red-600 text-white px-1 rounded font-bold"
                  >
                    {t}
                  </span>
                ))}
              ...
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 flex flex-col gap-2 transition-opacity">
                <ImageUpload
                  onUpload={(f) => handlePhotoUpload(f, item.id)}
                  className="bg-black/60 text-white p-1.5 rounded-full hover:bg-white hover:text-black"
                >
                  <Edit2 size={12} />
                </ImageUpload>
                <button
                  onClick={() => deleteBucketItem(item.id)}
                  className="bg-black/60 text-red-500 p-1.5 rounded-full hover:bg-red-600 hover:text-white"
                >
                  <Trash2 size={12} />
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#181818] border border-[#444] rounded-xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <p className="text-gray-400 text-xs mb-6">
              Choose when to add this episode.
            </p>
...
const TravelTab = ({ data, updateStore, currentUser }) => {
  ...
  const updateStay = (id, field, val) => {
    const newStays = data.stays.map((s) =>
    ...
    <div className="pb-24 px-4 pt-6 space-y-8">
      ...
      <section>
        ...
        <NetflixCard className="mb-4">
          <div className="p-4 flex gap-4">
            <DateBadge
              day="WED"
              date={data.flights.outbound.date}
              month={data.flights.outbound.month}
            />
        ...
        <NetflixCard className="mb-6">
          <div className="p-4 flex gap-4">
            <DateBadge
              day="SUN"
              date={data.flights.inbound.date}
              month={data.flights.inbound.month}
            />
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <span className="text-2xl font-black text-white">DPS</span>
        ...
        <div className="bg-[#181818] p-4 rounded-md border border-[#404040] relative">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={copyBooking}
              className="bg-[#333] text-white px-3 rounded hover:bg-[#444] transition-colors"
            >
              <Copy size={16} />
            </button>
          </div>
      ...
      <section>
        ...
        {data.stays.map((stay) => (
          <div
            key={stay.id}
            className="relative rounded-md overflow-hidden group bg-[#181818]"
          >
            <img
              src={stay.image}
...
const ExpensesTab = ({ data, updateStore, currentUser }) => {
  ...
  const [editingId, setEditingId] = useState(null); // ID of item currently being edited
  ...
  const totalMYR = myExpenses.reduce((acc, curr) => {
    const val =
      curr.currency === "IDR"
        ? curr.amount / data.config.currencyRate
  ...
  return (
    <div className="pb-24 px-4 pt-6 min-h-[80vh] flex flex-col">
      ...
      <div className="bg-gradient-to-br from-[#E50914] to-red-900 p-6 rounded-xl shadow-2xl mb-8 text-center relative overflow-hidden">
        <p className="text-red-200 text-xs font-bold tracking-widest uppercase mb-1">
          Total Spent
        </p>
        <h1 className="text-5xl font-black text-white tracking-tighter">
          {formatMYR(totalMYR).replace("RM", "")}
          <span className="text-lg opacity-50 font-medium">MYR</span>
        </h1>
        <button
      ...
      {viewHistory && (
        <div className="space-y-2 animate-in slide-in-from-bottom-4">
          {myExpenses.map((e) => (
            <div
              key={e.id}
              className="bg-[#181818] p-3 rounded-lg border border-[#333] flex justify-between items-center group"
            >
              {editingId === e.id ? (
                /* Edit Mode */
...
const BikesTab = ({ data, updateStore, currentUser }) => {
  ...
  const updateBike = (id, field, val) => {
    const updated = data.bikes.map((b) =>
      ...
      {myBikes.length === 0 ? (
        ...
        <div className="space-y-6">
          {myBikes.map((bike) => (
            <div
              key={bike.id}
              className="bg-[#181818] rounded-xl overflow-hidden border border-[#333]"
            >
              {/* Image Area */}
              <div className="h-48 bg-[#111] relative group">
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                  <ImageUpload
                    onUpload={(f) => handleBikePhoto(f, bike.id)}
                    className="bg-white text-black px-3 py-1.5 rounded text-xs font-bold"
                  >
                    Change Photo
                  </ImageUpload>
                </div>
                <button
                  ...
                  className="absolute top-2 right-2 bg-black/50 p-1 rounded-full text-red-500"
                >
                  <X size={14} />
                </button>
              </div>
              ...
              <div className="p-4 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={bike.plate}
                    onChange={(e) =>
                      updateBike(bike.id, "plate", e.target.value)
                    }
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {bike.mapLink ? (
                    <a
                      href={bike.mapLink}
                      target="_blank"
                      className="bg-[#333] hover:bg-[#444] text-white py-3 rounded-md font-bold text-sm flex items-center justify-center gap-2 transition-colors"
                    >
                      <Navigation size={16} /> Locate
                    </a>
                  ) : (
                    <div className="bg-[#222] text-gray-600 py-3 rounded-md font-bold text-sm flex items-center justify-center">
                      No Location
                    </div>
                  )}
                </div>
                {bike.parkedAt && (
                  <p className="text-[10px] text-gray-500 text-center">
                    Last parked: {new Date(bike.parkedAt).toLocaleTimeString()}
                  </p>
                )}
              </div>
            </div>
          ...
          <button
            onClick={addBike}
            className="w-full py-3 border border-dashed border-[#333] text-gray-500 hover:text-white hover:border-gray-500 rounded-lg text-sm transition-colors"
          >
            + Add Another Bike
          </button>
        </div>
...
const PackTab = ({ data, updateStore, currentUser }) => {
  ...
  const myItems = (data.packing || []).filter(
  ...
  const toggleCheck = (id) => {
    const updated = data.packing.map((i) =>
  ...
  const progress =
    myItems.length > 0
      ? Math.round(
          (myItems.filter((i) => i.checked).length / myItems.length) * 100
        )
      : 0;
    ...
    <div className="pb-24 px-4 pt-6">
      ...
      <div className="mb-6">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          ...
          <span>
            {myItems.filter((i) => i.checked).length}/{myItems.length}
          </span>
        </div>
        ...
        {myItems.map((item) => (
          <div
            ...
            className={`p-3 rounded-md flex items-center gap-3 cursor-pointer group transition-colors ${
              item.checked ? "bg-[#111]" : "bg-[#181818] hover:bg-[#222]"
            }`}
          >
            <div
      ...
      >
        {expanded ? (
          ...
          <div className="flex items-center gap-1 font-bold text-xs">
            <ChevronDown className="rotate-90" size={16} /> HELP
          </div>
        )}
      </button>
    </div>
  ...
  return (
    <div className="min-h-screen bg-black font-sans max-w-md mx-auto relative shadow-2xl overflow-hidden border-x border-[#1a1a1a] text-gray-200 selection:bg-red-900 selection:text-white">
      ...
      <div className="relative h-[45vh] w-full overflow-hidden">
        ...
        <img
          src={data.config.coverPhoto}
          className="w-full h-full object-cover animate-in fade-in zoom-in duration-1000"
        />

        {/* Navbar overlay */}
        <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-20">
          <div className="text-[#E50914] text-xl font-black tracking-tighter">
            N
          </div>
          <button
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-20 pb-12">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[#E50914] text-[10px] font-black border border-[#E50914] px-1 rounded-sm">
              SERIES
            </span>
          ...
          <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">
            Casting by: Edward · Jayden · Eugene · Daniel
          </p>
          ...
          <div className="flex gap-3 mt-4">
            <button
              ...
              className="bg-white text-black px-4 py-2 rounded font-bold text-sm flex items-center gap-2 hover:bg-gray-200"
            >
              <Calendar size={16} fill="black" /> Episodes
            </button>
            <button
              onClick={() => setActiveTab("pack")}
              className="bg-[#333]/80 backdrop-blur text-white px-4 py-2 rounded font-bold text-sm flex items-center gap-2 hover:bg-[#444]"
            >
      ...
      <div className="-mt-6 relative z-30 bg-black min-h-[50vh]">
        {activeTab === "plan" && (
          <PlanTab data={data} updateStore={updateStore} />
        )}
        {activeTab === "travel" && (
          <TravelTab
            data={data}
        ...
        {activeTab === "bucket" && (
          <BucketListTab data={data} updateStore={updateStore} />
        )}
        {activeTab === "pack" && (
          <PackTab
            data={data}
